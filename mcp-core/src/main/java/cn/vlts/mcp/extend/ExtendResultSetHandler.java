 ...
 * @author throwable
 * @version v1
 * @description 扩展Mybatis结果集处理器
 * @since 2023/12/25 15:01
 */
public class ExtendResultSetHandler implements ResultSetHandler {
    ...
    private static final String EXECUTOR_FIELD_KEY = "executor";

    private static final String ROW_BOUNDS_FIELD_KEY = "rowBounds";

    private static final String RESULT_HANDLER_FIELD_KEY = "resultHandler";
    ...
    private ExtendResultSetHandler(ResultSetHandler handler,
                                   Function<MultiKey, DuplexStringCryptoProcessor> cryptoProcessorLoader) {
        this.handler = handler;
        MetaObject metaObject = MybatisUtils.X.newMetaObject(handler);
        this.executor = (Executor) metaObject.getValue(EXECUTOR_FIELD_KEY);
        ...
        this.rowBounds = (RowBounds) metaObject.getValue(ROW_BOUNDS_FIELD_KEY);
        this.resultHandler = (ResultHandler<?>) metaObject.getValue(RESULT_HANDLER_FIELD_KEY);
    ...
    public static ExtendResultSetHandler newInstance(ResultSetHandler handler,
                                                     Function<MultiKey, DuplexStringCryptoProcessor> cryptoProcessorLoader) {
        return new ExtendResultSetHandler(handler, cryptoProcessorLoader);
    }
    ...
    private Object getPropertyMappingValue(ResultSet rs, MetaObject metaResultObject, ResultMapping propertyMapping,
                                           ResultLoaderMap lazyLoader, String columnPrefix, String resultMapId) throws SQLException {
        ...
        if (propertyMapping.getResultSet() != null) {
            ...
            if (Objects.equals(String.class, propertyMapping.getJavaType())) {
                DuplexStringCryptoProcessor cryptoProcessor
                        = cryptoProcessorLoader.apply(MultiKey.newInstance(resultMapId, propertyMapping.getProperty()));
            ...
            for (String columnName : unmappedColumnNames) {
                ...
                if (property != null && metaObject.hasSetter(property)) {
                    ...
                    if (Objects.equals(String.class, propertyType)) {
                        DuplexStringCryptoProcessor cryptoProcessor = cryptoProcessorLoader.apply(MultiKey.newInstance(resultMap.getId(), property));
                        if (Objects.nonNull(cryptoProcessor)) {
                            typeHandler = CryptoSupportTypeHandler.newInstance(cryptoProcessor);
                        }
                    }
                    if (Objects.isNull(typeHandler) && typeHandlerRegistry.hasTypeHandler(propertyType, rsw.getJdbcType(columnName))) {
    ...
    public <E> Cursor<E> handleCursorResultSets(Statement stmt) throws SQLException {
        return handler.handleCursorResultSets(stmt);
    }

    @Override
    public void handleOutputParameters(CallableStatement cs) throws SQLException {
        handler.handleOutputParameters(cs);
    }
}
